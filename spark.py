...
cursor = conn.cursor()
strQuery ="select top 100 KEY_VALUE,STOCK_CODE from ydzq..tbl_user_stock"
...
conf = SparkConf().setAppName("Hello").setMaster("spark://10.237.2.130:7077").set("spark.executor.memory","9g").set("spark.default.parallelism","2").set("spark.driver.memory","9g").set("spark.driver.maxResultSize","8g")

sc = SparkContext(conf=conf)


usrTusr_Accum = sc.accumulator({}, usrTusr_AccumulatorParam())
long_Accum = sc.accumulator(0)

rdd_stkUsr=sc.parallelize(dict_to_list(stkUsr))

rdd_stkUsr.foreach(lambda x:add(x,usrTusr_Accum))
rdd_stkUsr.foreach(lambda x:addl(x,long_Accum))
#print sc.parallelize(dict_to_list(stkUsr),250).collect()

usrTusr = usrTusr_Accum.value
count = long_Accum.value

print('usrTusr',usrTusr,'long_Accum',count)
